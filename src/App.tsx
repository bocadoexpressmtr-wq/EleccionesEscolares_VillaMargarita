...
const AdminDashboard = ({ onLogout }: { onLogout: () => void }) => {
  ...
  // Config is always needed globally in the admin panel
  useEffect(() => {
  ...
  // Candidates listener - only load when needed
  useEffect(() => {
    if (!['candidates', 'results', 'jornada'].includes(activeTab)) return;
  ...
  // Mesas listener - only load when needed
  useEffect(() => {
    if (!['mesas', 'results', 'jornada'].includes(activeTab)) return;
  ...
  // Admin emails listener - only load when needed
  useEffect(() => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        if (info.isQuota) {
          // Try loading from cache if quota is exceeded
          try {
            ...
            // If cache also fails, then throw the quota error
            throw new Error(JSON.stringify(info));
          }
        } else {
          throw new Error(JSON.stringify(info));
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md bg-white p-12 rounded-3xl shadow-2xl">
          ...
          <div className="flex flex-col gap-3">
            <button 
              ...
              className="text-slate-300 hover:text-slate-500 transition-colors text-xs py-2 mt-4"
            >
              Configurar Mesa
            </button>
          </div>
